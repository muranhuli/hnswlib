...
namespace hnswlib
{
    ...
    public:
        ...
        struct SuperNode
        {
            ...
            std::unordered_map<int, std::vector<size_t>> cluster;
            dist_t radius;
            ...
            void add_point(tableint point_id, const void* data_point)
            {
                ...
                update_radius();
                // 更新超点的簇，属于哪一个聚类
                int hashValue = parent->lsh_->getHashValue(data_point);
                // std::cout<<hashValue<<std::endl;
                cluster[hashValue].emplace_back(contain_points_list.size() - 1);
            }
              ...
              allow_replace_deleted_(allow_replace_deleted)
        {
            ...
            lsh_ = new LSH(*((size_t*)dist_func_param_), 1000);
        ...
        void hnsw_graph_info_stats() const
        {
            ...
            for (size_t i = 0; i < cur_super_node_count; i++)
            {
                ...
                sum_edge += size;
                if (super_node_list_.at(i)->cluster.size()>=2)
                {
                    for (auto info: super_node_list_.at(i)->cluster)
                    {
                        std::cout<<info.first<<" "<<info.second.size()<<";";
                    }
                    std::cout<<std::endl;
                }
            }
        ...
        // 计算超点中心点之间的距离
        inline void distance(const void* data, tableint* superNodedata, const int size, dist_t* v) const
        {
        ...
        {
            // 确定需要查询的超点的簇，距离超点中已有簇的距离最近
            int cluster_id = -1;
            int dis_id = INT_MAX;
            for (auto&& pair : super_node_list_.at(id2)->cluster)
            {
                if (std::abs(pair.first - hashValue) < dis_id)
                {
                    dis_id = std::abs(pair.first - hashValue);
                    cluster_id = pair.first;
                }
            }

            dist_t dist = INT_MAX;

            auto& list2 = super_node_list_.at(id2)->cluster.at(cluster_id);
            ...
            for (size_t j = 0; j < list2.size(); j = j + 13)
            {
                ...
                for (size_t k = 0; k < len; ++k)
                {
                    pVects[k] = super_node_list_.at(id2)->contain_points_list[list2[j + k]].second.data();
                }
                for (auto& reg : regs)
                {
                    reg = _mm256_set1_ps(0.0f);
                ...
                for (size_t k = 0; k < len; ++k)
                {
                    float PORTABLE_ALIGN32 TmpRes[8];
                    _mm256_store_ps(TmpRes, regs[k]);
                    v[list2[j + k]] = TmpRes[0] + TmpRes[1] + TmpRes[2] + TmpRes[3] + TmpRes[4] + TmpRes[5] +
                        TmpRes[6] + TmpRes[7];
                    dist = std::min(dist, v[list2[j + k]]);
                }
            }
            ...
            BaseSearchStopCondition<dist_t>* stop_condition = nullptr) const
        {
            ...
            int hashValue = lsh_->getHashValue(data_point);
            if (bare_bone_search ||
                (!isMarkedDeleted(ep_id) && ((!isIdAllowed) || (*isIdAllowed)(getExternalLabel(ep_id)))))
            {
                auto res_pair = distance(data_point, ep_id, this->node_dist, hashValue);
                lowerBound = std::min(lowerBound, std::get<1>(res_pair));
                int index = std::get<0>(res_pair);
                for (auto nodeIndex: this->super_node_list_.at(ep_id)->cluster.at(index))
                {
                    top_candidates.emplace(node_dist[nodeIndex],
                                           this->super_node_list_.at(ep_id)->contain_points_list[nodeIndex].first);
                }


                // for (size_t i = 0; i < this->super_node_list_.at(ep_id)->cluster.at(index).size(); i++)
                // {
                //     int nodeIndex = this->super_node_list_.at(ep_id)->cluster.at(index).at(i);
                //     top_candidates.emplace(node_dist[nodeIndex],
                //                            this->super_node_list_.at(ep_id)->contain_points_list[nodeIndex].first);
                // }
                ...
                for (size_t j = 0; j < size; j++)
                {
                    ...
                    if (!(visited_array[candidate_id] == visited_array_tag))
                    {
                        ...
                        if (top_candidates.size() < ef || lowerBound > dist)
                        {
                            ...
                            auto res_pair = distance(data_point, candidate_id, this->node_dist, hashValue);
                            dist = std::get<1>(res_pair);
                            index = std::get<0>(res_pair);
                        }
                        else
                        {
                            num1++;
                        ...
                        if (flag_consider_candidate)
                        {
                            ...
                            {
                                for (auto nodeIndex: this->super_node_list_.at(candidate_id)->cluster.at(index))
                                {
                                    if (top_candidates.size() < ef || lowerBound > node_dist[nodeIndex])
                                    {
                                        top_candidates.emplace(node_dist[nodeIndex],
                                                               this->super_node_list_.at(candidate_id)->contain_points_list[nodeIndex].first);
                                    }
                                }

                                // for (size_t i = 0; i < this->super_node_list_.at(candidate_id)->cluster.at(index).size(); i++)
                                // {
                                //     int nodeIndex = this->super_node_list_.at(candidate_id)->cluster.at(index).at(i);
                                //     if (top_candidates.size() < ef || lowerBound > node_dist[nodeIndex])
                                //     {
                                //         top_candidates.emplace(node_dist[nodeIndex],
                                //                                this->super_node_list_.at(candidate_id)->contain_points_list[nodeIndex].first);
                                //     }
                                // }
                                // for (size_t i = 0; i < this->super_node_list_.at(candidate_id)->contain_points_list.
                                //                              size(); i++)
                                // {
                                //     if (top_candidates.size() < ef || lowerBound > node_dist[i])
                                //     {
                                //         top_candidates.emplace(node_dist[i],
                                //                                this->super_node_list_.at(candidate_id)->
                                //                                      contain_points_list[i].first);
                                //     }
                                // }
        ...
        */
        ...
        tableint addPoint(const void* data_point, labeltype label, int level)
        {
            ...
            memset(data_level0_memory_ + cur_superNode * size_data_per_element_ + offsetLevel0_, 0,
